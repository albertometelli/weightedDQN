        ...
        if absorbing:
            ...
            if self._update_mode == 'deterministic':
                if self._update_type == 'mean':
                    best = np.random.choice(np.argwhere(mean_next_all == np.max(mean_next_all)))
                    ...
                    sigma_next = np.sqrt(np.sum((sigma_next_all * prob)**2))
                ...
                self.Q.model[1][state, action] = sigma + self.alpha[1](state, action) * (
                        reward + self.mdp_info.gamma * sigma_next - sigma)
            else:
                raise NotImplementedError()
        ...
        if absorbing:
            ...
            if self._update_mode == 'deterministic':
                if self._update_type == 'mean':
                    ...
                    sigma_next = np.sqrt(np.sum((sigma_next_all_2 * prob) ** 2))
