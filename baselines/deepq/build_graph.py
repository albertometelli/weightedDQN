...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        '''for a in range(num_actions):
            q_samples.append(tf.random.normal(tf.stack([batch_size]),mean=q_values[a], stddev=sigma_values[a]))
        '''
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        distributions = []
        max_prob = []
        action_max_prob_functions = []
        batch_size = tf.shape(obs_t_input.get())[0]
        for a in range(num_actions):
            distributions.append(tf.distributions.Normal(loc=q_tp1[:, a], scale=sigmas_tp1[:, a]))
        def make_function(action,i):
            def f(y, x):
                p = 1
                p *= distributions[action].prob(x)[i]
                for k in range(num_actions):
                    if k != action:
                        p *= distributions[k].cdf(x)[i]
                return p
            return f
        ...
        qs_and_sigmas_target = tf.stack([q_tp1, sigmas_tp1], axis=2)

        '''for a in range(num_actions):

            batch = []
            unstacked_q = tf.unstack(q_tp1)
            unstacked_sigma = tf.unstack(sigmas_tp1)
            count = tf.constant(0, dtype=tf.int32)
            for q_row in unstacked_q:
                sigma_row = unstacked_sigma[count]
                f = make_function(a, count)
                batch.append(tf.contrib.integrate.odeint_fixed(f, y0=0,
                                                     t=tf.stack([q_row[a] - 8 * sigma_row[a], q_row[a] + 8 * sigma_row[a]]), method='rk4'))
                count += 1
            max_prob.append(tf.stack(batch))'''

        tensor_type = tf.placeholder(tf.float32, [num_actions])
        prob = tf.map_fn(integrate, qs_and_sigmas_target,dtype=tensor_type.dtype)

        q_target = rew_t_ph + gamma * tf.reduce_sum(tf.multiply(prob, q_tp1), 1)
        sigma_target = gamma * tf.reduce_sum(tf.multiply(prob, sigmas_tp1), 1)

        # compute the error (potentially clipped)
        td_error = (q_t_selected - tf.stop_gradient(q_target)) ** 2 +\
                   (sigma_t_selected - tf.stop_gradient(sigma_target)) ** 2
        ...
        train = U.function(
            ...
            outputs=[td_error, q_t, sigma_t, q_tp1, sigmas_tp1, prob, q_target],
